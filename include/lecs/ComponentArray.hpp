...
template <typename T> class ComponentArray : public IComponentArray {
  ...
  public:
    ...
    void Remove(Entity ent) {
        ...
        if (m_Size == 0 || !m_HasComponent[ent])
            return;

        size_t index =
            m_EntityToIndex[ent];      // Index of the entity to be deleted
        ...
        m_HasComponent[ent] = false;
        m_Size--;

        // std::cout << "-- Component " << typeid(T).name() << " Ent " << ent <<
        // " MOVE from " << lastEntity << '\n';
    }

    T& Get(Entity ent) { return m_Components[m_EntityToIndex[ent]]; }

    T& Get(size_t index) { return m_Components[index]; }
