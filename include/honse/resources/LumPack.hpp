...
class LumPack {
    public:
        explicit LumPack(std::string path) : m_Path(std::move(path)) {}

        LumAssetHandle Extract(std::string id) {
            return LumAssetHandle { lum_extract_asset(m_Path.c_str(), lum_hash(id.c_str())) };
        }
...
class LumPacks {
    public:
        static void Mount(std::string path) { m_Packs.emplace_back(std::move(path)); }
        ...
        static const char* Path(std::string name) {
            ...
            for (auto pack = m_Packs.rbegin(); pack != m_Packs.rend(); ++pack) {
                ...
                if (asset) {
                    auto [inserted, _] = m_Extracted.emplace(id, std::move(asset));
